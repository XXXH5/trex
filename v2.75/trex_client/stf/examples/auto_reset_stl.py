import time
import datetime
from examples.stf_path import *
from trex_stf_lib.trex_client import CTRexClient

from get_active_hosts import get_active_host
from trex_stf_lib.trex_status_e import TRexStatus

def restart_daemon_server(ip):
    try:
        trex = CTRexClient(ip)
        if trex.get_running_status()['state'] == TRexStatus.Idle:
            #print('trex server process is not running')
            return False
        else:
            trex.force_kill(confirm=False)
            time.sleep(2)  # 4
            if trex.get_running_status()['state'] == TRexStatus.Idle:
                print("Success killed server, ip:" + ip)
                return True
    except Exception as e:
        print('error occurs when automatically resetting up trex stateless server at: ' + ip)
        print(e)
        pass


def auto_reset_daemon_server(ip_list, n=None):
    if not n:
        n = len(ip_list)
    host_killed = []
    for i in ip_list[:n]:
        if restart_daemon_server(i):
            host_killed.append(i)
        elif not restart_daemon_server(i):
            print('server ' + i + ' is not running , can not be killed')
    if (len(host_killed)) > 0:
        print("trex nodes has been killed: {0}".format(len(host_killed)))
        for i in host_killed:
            print('ip: ' + i)


if __name__ == '__main__':
    network = '192.168.1'
    n = 100
    print('Scanning reachable ips')
    ips = get_active_host(network, n)
    # filter gateways
    #ips.remove("192.168.115.1")
    #ips.remove("192.168.115.2")
    #ips.remove("192.168.115.129")
    ips.remove("192.168.1.30")
    # show the result ips
    print(ips)
    # start statelesss servers
    auto_reset_daemon_server(ips)
# restart_daemon_server('192.168.115.130')
